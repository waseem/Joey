module Joey
  class Photo < Model
    ...
    def validate
      ...
      errors << { :message => "height is neither string nor nil but is #{height.inspect}" } unless height.is_a?(Integer) || height.nil?
      errors << { :message => "icon is neither string nor nil but is #{icon.inspect}" } unless icon.is_a?(String) || icon.nil?
      errors << { :message => "width is neither string nor nil but is #{width.inspect}" } unless width.is_a?(Integer) || width.nil?
